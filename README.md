...
flowchart TB
    ...
    subgraph Orchestration["编排层 — AgentCore (app/agent/core.py)"]
        ...
        LLMRouter["LLMQueryRouter\nLLM 路由（function calling）"]
    ...
    subgraph Synthesis["校验与合成层"]
        ...
        Compose["_compose_structured_answer\n编排结构化 blocks\n(chart/table/bullets/warning/quote)"]
        LLM["DeepSeek\n(OpenAI 兼容接口)\n基于工具数据生成分析文本"]
        RG["ResponseGuard\nLLM 输出数值溯源校验"]
    end
...
| 层级 | 职责 | 核心组件 | 对应代码 |
|------|------|----------|----------|
| **接入层** | 接收 HTTP 请求，以 SSE 协议向前端推送事件流 | FastAPI、sse-starlette | `app/api/routes_module.py` |
| **编排层** | 意图分类、实体提取、工具选择、并发分组 | QueryRouter（内含 QueryEnricher.extract_symbols）、LLMQueryRouter、AgentCore | `app/agent/core.py`、`app/routing/`、`app/enricher/` |
| **执行层** | 并发调用外部工具，Stage 0（行情）优先于 Stage 1（增强） | MarketDataService、HybridRAGPipeline、WebSearchService、SECFilingsService | `app/market/`、`app/rag/`、`app/search/` |
| **数据层** | 行情缓存（Redis）、知识库（ChromaDB）、外部 API | Redis、ChromaDB、yfinance、akshare | `app/cache/`、`app/rag/pipeline.py` |
| **校验与合成层** | 数据校验 → 技术指标计算 → blocks 编排 → LLM 文本生成 → 输出数值溯源 | DataValidator、TechnicalAnalyzer、`_compose_structured_answer`、DeepSeek、ResponseGuard | `app/analysis/`、`app/agent/core.py` |
...
③ 路由决策：
   ...
   - 行情/混合类                                → LLMQueryRouter（function calling）选择工具 + 参数
      ↓
④ _build_tool_plan：将工具按依赖关系编排为 Stage 0（行情）/ Stage 1（增强）两批
      ↓
...
⑩ DeepSeek（generator prompt + hard_guardrails）：
   接收 _compose_structured_answer 返回的 final_text（工具数据的文本化摘要）+ RAG 文档片段
   仅生成纯文字分析结论，不参与数值计算，不重复已在 blocks 中展示的数字
   → 流式推送 chunk 事件（打字机效果）
      ↓
⑪ ResponseGuard.validate()：
   校验 LLM 输出中的数值是否均可溯源至工具 payload，防止编造
      ↓
...
| **数据与排版分离** | blocks 事件早于 chunk 事件推送 | 客观数据由 `_compose_structured_answer` 编排为原生 blocks 组件（chart/table/bullets），LLM 只负责写文字结论，不重复数字；彻底杜绝 Markdown 表格错乱，前端秒级出图 |
| **双路由策略** | 规则路由 + LLM 路由 | 规则路由保证行情类 100% 走 API（不依赖 LLM 判断）；LLM 路由处理模糊意图；降级时规则路由兜底 |
| **双重反幻觉** | DataValidator + ResponseGuard | DataValidator 校验「有数可答」；ResponseGuard 校验「数值来自工具」；从输入和输出两端封堵幻觉 |
...
| **反幻觉机制** | DataValidator + ResponseGuard | **DataValidator**：校验工具返回是否满足最低要求（如无价格则拒答）；**ResponseGuard**：校验 LLM 输出中的数值是否均来自工具 payload，防止编造。 | 纯 LLM 自检：不可靠；外部 Guardrails API：增加延迟与依赖。 |
...
**目录**：`backend/data/knowledge/`（共 11 个 Markdown 文件）

这是系统 RAG 检索的核心语料库，由人工整理的结构化金融知识文档构成，经 MinerU 解析 / 直接编写后写入向量库，供 `search_knowledge` 工具检索。

| 文件 | 内容摘要 |
|------|----------|
| `股票基础.md` | A股/港股/美股交易规则、涨跌停板、T+0/T+1、融资融券、股票质押 |
| `core_finance_metrics.md` | PE、PB、EPS、PEG、ROE、EV/EBITDA、FCF 等核心估值指标定义与公式（含 YAML 元数据标注） |
| `基本面分析.md` | 财务三表解读、盈利能力、偿债能力、成长性分析框架 |
| `技术分析.md` | K线形态、均线系统、MACD、RSI、布林带等技术指标原理 |
| `ETF基金.md` | ETF 分类、折溢价、跟踪误差、LOF、QDII 等知识 |
| `宏观经济.md` | GDP、CPI、利率、汇率、货币政策对市场的传导机制 |
| `市场术语.md` | 常见金融术语词典，覆盖 A股、港股、美股市场 |
| `投资策略.md` | 价值投资、成长投资、量化策略、资产配置理论 |
| `量化投资.md` | 因子模型、回测逻辑、Alpha 挖掘、策略评估指标 |
| `期权期货.md` | 衍生品定价原理（B-S 模型）、Greeks、套期保值、套利策略 |
| `风险管理.md` | VaR、最大回撤、夏普比率、凯利公式、仓位管理原则 |
...
| **知识库覆盖窄** | 11 个 Markdown 文件，约 3 万字，缺乏 A 股研报、宏观数据、行业报告 | 人工整理成本高，没有自动更新机制 |
...
当前 11 个 Markdown 文件构成的静态知识库存在明显的覆盖盲区：没有行业研报、没有宏观数据、没有 A 股上市公司的具体信息。生产环境下需要三方面升级：

**① 内容规模化**

```
当前：11 个手写 MD 文件 ≈ 3 万字
目标：
...
当前系统无监控，生产环境中任何工具失败、LLM 超时、幻觉漏网都是黑盒。
...
以下是因时间限制未在本次 MVP 实现，但具有明确价值的创新点：
...
无监控                 →   Prometheus + Grafana 基础监控  →  完整 APM + 合规审计
单机 ChromaDB          →   向量库集群化（Weaviate）        →  Graph RAG（知识图谱）
MVP 合规（Prompt 层）  →   独立合规审查器                  →  监管对接 + 持牌运营
被动问答               →   ————————————————————————————  →  主动预警 + 推送订阅
```
...
│   │   └── api/routes_module.py     # FastAPI 路由（/api/chat SSE 端点）
