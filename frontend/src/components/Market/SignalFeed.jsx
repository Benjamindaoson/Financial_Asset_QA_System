...
export function SignalFeed({ signals }) {
  if (!signals || signals.length === 0) {
    return <div style={{ padding: "20px", color: C.td }}>No signals detected.</div>;
  }
...
function SignalCard({ signal }) {
  ...
  const signalStrength = signal.signal_score >= 80 ? "Strong" : signal.signal_score >= 60 ? "Medium" : "Watch";
